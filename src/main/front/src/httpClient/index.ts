...
export async function logout() {
    await fetch(`${BASE_URL}/logout`, getRequestInit);
}
...
export async function getParticipatingCards(): Promise<CardViewModel[]> {
    const response = await fetch(`${BASE_URL}/api/card/getParticipating`, getRequestInit);
    if (!response.ok) {
        throw new Error(await response.text())
    }
...
export async function addCard(card: CardAddOrEditModel): Promise<CardViewModel> {
...
export async function editCard(card: CardAddOrEditModel, id: number): Promise<CardViewModel> {
